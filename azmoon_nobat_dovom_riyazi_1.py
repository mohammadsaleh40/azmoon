...
first_text = add_question(first_text , "عبارت رادیکالی زیر را به شکل توان گویا ساده کنید. \\\\ $\\sqrt[4]{16}\\times\\sqrt[2]{9} = $" , khat= 1 , barom = 1)

first_text = add_question(first_text , "معادله زیر را حل کنید. \\begin {LTR}$x^2+6x = 2$  \\end{LTR}" , barom= 1.5 , khat= 1)

#first_text = add_question(first_text , "نا معادله زیر را حل کنید.", ["$|\\dfrac{x-1}{2}-1| \\geq 0$" , "$|\\dfrac{x-1}{2}-1| \\geq 3$" , "$x+1 \\leq 5-x < 2x +3$"] , ltr = True )
...
first_text = add_section(first_text , "دانش‌ آموزان عزیز جهت کسب 6 نمره از سؤالات 13 تا 19 فقط ۳ سؤال را به دلخواه انتخاب و پاسخ دهید.")

first_text = add_question(first_text , """یک فروشگاه دو نوع کارت اعتباری A و B را می‌پذیرد. اگر 34 درصد از مشتریان کارت نوع A $(P(A) = \dfrac{34}{100})$ و 62 درصد کارت نوع B و 15 درصد هر دو کارت را همراه داشته باشند، چقدر احتمال دارد مشتریان با در اختیار داشتن حداقل یکی از این دو کارت از این فروشگاه خرید کنند؟""" , khat= 2 , barom= 2)
