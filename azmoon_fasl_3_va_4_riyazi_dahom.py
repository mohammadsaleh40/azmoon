...
first_text = add_fourchoice(first_text , "عدد 4- چند ریشه 8 ام دارد؟", ["0", "1", "2", "8"])
first_text = add_question(first_text , "در چه مواردی ریشه گیری باعث کوچک شدن مقدار عددی عدد اولیه می‌شود؟", khat = 2)
first_text = add_question(first_text , "در چه مواردی ریشه گیری باعث بزرگ شدن مقدار عددی عدد اولیه می‌شود؟", khat = 2)
...
["$\\sqrt[7]{5^3} = $", "$(4)^{\\dfrac{5}{6}} = $" , "$(12)^{\\dfrac{5}{3}} = $" , "$(-\\dfrac{3}{7})^{\\dfrac{5}{8}} = $"], ltr = True)
